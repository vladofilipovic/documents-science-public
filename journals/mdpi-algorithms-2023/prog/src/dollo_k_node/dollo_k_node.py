"""
The :mod:`dollo_k_node` module contains DolloKNode class.

DolloKNode class is an node of the mutation tree to be build and evaluated.
"""
...
class DolloKNode(EaNode):
    ...
    def is_correct(self, labels, dollo_k):
        ...
        Returns:
            ...
            and second component is label that causes problem.
        """
        for l in labels:
            p_l = l + '+'
    ...
    def tree_initialize(self, labels, dollo_k, max_fill_factor):
        ...
        Args:
            ...
            max_fill_factor (:float): Parameter `max_fill_factor` represents fill factor to multiply to get max tree size.         
        ...
        current_tree_size = 1
        max_size = len(labels) + int(random.random()*len(labels)*max_fill_factor)  
        i=1
        while((i<= max_size) or len(plus_not_used)>0 ): 
            ...
            if(random.randrange(max_size)<len(plus_not_used)):
                ...
                while(iterations <= len(labels)):
                    ...
                    leaf = DolloKNode(label_to_insert+'+', leaf_bit_array)
                    # attach leaf node

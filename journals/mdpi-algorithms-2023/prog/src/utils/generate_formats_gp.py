...
def anytree_node_name(node):
    return '%s:%s' % (node.node_label, node.depth)

def anytree_edge_attribute(node, child):
    return 'label="%s:%s"' % (node.node_label, child.node_label)

def anytree_edge_type(node, child):
    return '--'
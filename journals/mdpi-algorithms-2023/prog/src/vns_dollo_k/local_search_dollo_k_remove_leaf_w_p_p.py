""" The :mod:`local_search_dollo_k_remove_leaf_w_p_p` module contains vns
...
class LocalSearchDolloKRemoveLeafWPP():
    @staticmethod
    def improve_solution(solution_to_improve: DolloKSolution, alpha: float, beta: float,
        ...
        logger.debug("solution_to_improve at begin of LocalSearchDolloKRemoveLeafWPP:" )
        ...
        nodes = list(PreOrderIter(individual))
        selected = []
        for node in nodes:
            if not node.is_leaf:
                for child in node.children:
                    for grandchild in child.children:
                        if grandchild.is_leaf and not(node in selected):

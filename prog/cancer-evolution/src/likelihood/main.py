...
from read_input import read_labels_reads_format_in
...
def main():
        ...
        if(len(sys.argv)==1):
                ...
                seed=int(sys.argv[4])
        ex='\A(\w+): ((0|1)+)'
        (mutations,matrix) =read_labels_reads_format_in(f)
        ...
        for i in new_tree:
                logger.debug("parent : ",i[0].node_label," child : ",i[1]," likelihood_node : ",i[2])
        logger.debug("valore di likelihood per l'albero : ",likelihood_tree)

""" The :mod:`local_search_dollo_k_remove_leaf_w_p` module contains vns
...
class LocalSearchDolloKRemoveLeafWP():
    @staticmethod
    def improve_solution(solution_to_improve: DolloKSolution, alpha: float, beta: float,
        ...
        logger.debug("solution_to_improve at begin of LocalSearchDolloKRemoveLeafWP:" )
        ...
        for node in nodes:
            if not node.is_leaf:
                for child in node.children:
                    if child.is_leaf and not (node in selected) :
